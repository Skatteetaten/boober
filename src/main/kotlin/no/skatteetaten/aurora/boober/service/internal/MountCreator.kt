...
private fun createMountsFromDeploymentSpec(deploymentSpecInternal: AuroraDeploymentSpecInternal): List<Mount> {
    ...
    val certMount = deploymentSpecInternal.integration?.certificateCn?.let {
        Mount(path = "/u01/secrets/app/${deploymentSpecInternal.name}-cert",
            type = MountType.Secret,
    ...
    return listOf<Mount>().addIfNotNull(secretVaultMount).addIfNotNull(configMount).addIfNotNull(certMount).addIfNotNull(deploymentSpecInternal.volume?.mounts)
}
