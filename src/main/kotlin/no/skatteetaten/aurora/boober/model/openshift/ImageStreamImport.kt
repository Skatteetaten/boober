...
data class From(val kind: String = "",
                val name: String = "")
...
@JsonInclude(JsonInclude.Include.NON_NULL)
data class Import(val metadata: Metadata? = null,
                  val spec: Spec? = null,
                  val status: ImportStatus? = null)


data class ImportStatus(val dockerImageRepository: String = "",
                        val tags: List<TagsItem> = emptyList())


data class TagsItem(val tag: String = "",
                    val items: List<ItemsItem>? = emptyList(),
                    val conditions: List<ConditionsItem> = emptyList())


data class ItemsItem(val generation: Int = 0,
                     val image: String = "",
                     val created: String = "",
                     val dockerImageReference: String = "")


data class ConditionsItem(val type: String = "",
                          val status: String = "",
