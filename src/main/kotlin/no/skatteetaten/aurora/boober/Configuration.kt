...
class Configuration {
    ...
    @Bean
    fun msGraphApiClient(
            @Value("\${integrations.azure.clientId}") clientId: String,
            @Value("\${integrations.azure.tenantId}") tenant: String,
            @Value("\${integrations.azure.secretKey}") clientSecret: String
    ): IGraphServiceClient {

        val authProvider = ClientCredentialProvider(clientId, listOf("https://graph.microsoft.com/.default"), clientSecret, tenant, NationalCloud.Global)
